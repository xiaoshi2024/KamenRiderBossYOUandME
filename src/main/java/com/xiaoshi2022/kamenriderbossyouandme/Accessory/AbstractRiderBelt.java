 ...
 */
public abstract class AbstractRiderBelt extends Item implements ICurioItem {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        // 检查玩家是否已经装备了腰带
        var existingBeltOpt = CurioUtils.findFirstCurio(serverPlayer,
                stack -> stack != null && stack.getItem() instanceof AbstractRiderBelt);

        // 如果已有腰带，将其替换回背包
        if (existingBeltOpt.isPresent()) {
            var slotResult = existingBeltOpt.get();
