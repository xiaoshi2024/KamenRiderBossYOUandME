...
public class BaronBananaEnergyEntity extends LivingEntity implements GeoEntity {
    ...
    public BaronBananaEnergyEntity(EntityType<? extends LivingEntity> type, Level world) {
        super(type, world);
        this.noPhysics = true;
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide()) {
            ...
            for (Entity nearbyEntity : this.level().getEntities(this,
                    ...
                    entity -> entity instanceof LivingEntity && !(entity instanceof BaronBananaEnergyEntity))) {
                ...
                if (nearbyEntity instanceof LivingEntity livingEntity) {
                    ...
                    // 添加香蕉能量的特殊粒子效果（可以在客户端处理）
                    spawnBananaParticles(livingEntity);
                    }
                }
            }
        }
    ...
    // 生成香蕉能量粒子效果
    private void spawnBananaParticles(Entity target) {
        if (this.level().isClientSide()) {
            for (int i = 0; i < 3; i++) {
    ...
    private PlayState predicate(AnimationState<BaronBananaEnergyEntity> event) {
