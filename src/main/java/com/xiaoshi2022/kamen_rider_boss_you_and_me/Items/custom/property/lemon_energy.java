...
public class lemon_energy extends Item implements GeoItem {
            ...
            if (!stack.getOrCreateTag().contains("first_click")) {
                ...
                if (level.isEmptyBlock(aboveHead)) {
                    level.setBlock(aboveHead,
                            ModBlocks.LEMON_BLOCK.get().defaultBlockState(),
                            Block.UPDATE_ALL);
                ...
                player.displayClientMessage(Component.literal("再次点击装备柠檬能量锁种"), true);
    ...
    private void playSound(Level level, Player player, BlockPos pos) {
        if (level instanceof ServerLevel serverLevel) {
            serverLevel.playSound(null, pos, ModBossSounds.LEMON_ENERGY.get(), SoundSource.PLAYERS, 1, 1);
        }
    }
                ...
                .setSoundKeyframeHandler(state -> {
                    ...
                    if (player != null) {
                        if (state.getController().getCurrentAnimation() != null &&
                            ...
                            player.playSound(ModBossSounds.LEMON_ENERGY.get(), 1, 1);
                        }
                    }
                }));
    }
