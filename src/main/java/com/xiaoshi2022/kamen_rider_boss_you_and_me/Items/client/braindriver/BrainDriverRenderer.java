...
public class BrainDriverRenderer extends GeoItemRenderer<BrainDriver> implements ICurioRenderer {
    private final GeoModel<BrainDriver> defaultModel = new BrainDriverModel(new ResourceLocation(kamen_rider_boss_you_and_me.MODID, "brain_driver"));
    private final GeoModel<BrainDriver> brainRiderModel = new BrainDriverBrainModel(new ResourceLocation(kamen_rider_boss_you_and_me.MODID, "brain_driver_brain"));

    public BrainDriverRenderer() {
        super(new BrainDriverModel(new ResourceLocation(kamen_rider_boss_you_and_me.MODID, "brain_driver")));
    }
    ...
    public GeoModel<BrainDriver> getGeoModel() {
        ...
        if (stack.getItem() instanceof BrainDriver) {
            BrainDriver belt = (BrainDriver) stack.getItem();
            BrainDriver.BeltMode mode = belt.getMode(stack);

            switch (mode) {
            case BRAIN:
                return brainRiderModel; // 返回Brain骑士形态模型
            default:
