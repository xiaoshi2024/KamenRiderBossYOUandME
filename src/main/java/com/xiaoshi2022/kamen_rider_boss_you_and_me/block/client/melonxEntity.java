...
public class melonxEntity  extends BlockEntity implements GeoBlockEntity {
    //定义动画
    private static final RawAnimation OPENZIPB = RawAnimation.begin().thenPlayAndHold("openzipb");


    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);
    int lifespan = 600; // 40 tick lifespan (2 seconds)

    public melonxEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.MELONSX_ENTITY.get(), pos, state);
    }
    ...
    public static void tick(Level level, BlockPos pos, BlockState state, melonxEntity entity) {
