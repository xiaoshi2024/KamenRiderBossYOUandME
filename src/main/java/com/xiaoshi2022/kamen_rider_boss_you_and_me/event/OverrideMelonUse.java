...
public class OverrideMelonUse {
    ...
    public static void onRightClickItem(PlayerInteractEvent.RightClickItem event) {
        ...
        // 1. 必须是 weapon 的 Melon
        if (!(stack.getItem() instanceof Melon)) return;
        ...
        if (belt.isPresent()) {
            ...
            if (!tag.contains("first_click")) {
                ...
                if (level.isEmptyBlock(above)) {
                    level.setBlock(above,
                            ...
                            /* 也可换成蜜瓜专属方块 */
                            ModBlocks.MELON_BLOCK.get().defaultBlockState(),
                            3);
                    level.playSound(null, above,
                            ModSounds.MELONENERGY.get(),
                            SoundSource.PLAYERS, 1, 1);
                }
                player.displayClientMessage(Component.literal("再次点击装备蜜瓜锁种"), true);
            } else {
                // 第二次右键
            ...
            InteractionResultHolder<ItemStack> result =
                    ((Melon) stack.getItem()).use(level, player, hand);
