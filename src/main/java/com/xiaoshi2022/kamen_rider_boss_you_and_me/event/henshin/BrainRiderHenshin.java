...
public final class BrainRiderHenshin {

    private BrainRiderHenshin() {}

    /** 穿 3 件Brain装甲：头盔、胸甲、护腿 */
    public static void trigger(Player player) {
        // 保存原盔甲（4 个槽位）
        ItemStack[] original = saveArmor(player);

        // 穿上Brain装甲
        player.setItemSlot(EquipmentSlot.HEAD,  new ItemStack(ModItems.BRAIN_HELMET.get()));
        player.setItemSlot(EquipmentSlot.CHEST, new ItemStack(ModItems.BRAIN_CHESTPLATE.get()));
        player.setItemSlot(EquipmentSlot.LEGS,  new ItemStack(ModItems.BRAIN_LEGGINGS.get()));
        // 清空鞋子槽位
