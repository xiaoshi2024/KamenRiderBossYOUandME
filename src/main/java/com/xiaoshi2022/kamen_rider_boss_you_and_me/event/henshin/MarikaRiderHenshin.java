...
public final class MarikaRiderHenshin {

    private MarikaRiderHenshin() {}

    /** 穿 3 件Marika装甲：头盔、胸甲、护腿 */
    public static void trigger(Player player) {
        // 保存原盔甲（3 个槽位）
        ItemStack[] original = saveTop3(player);

        // 穿上Marika装甲
        player.setItemSlot(EquipmentSlot.HEAD,  new ItemStack(ModItems.MARIKA_HELMET.get()));
        player.setItemSlot(EquipmentSlot.CHEST, new ItemStack(ModItems.MARIKA_CHESTPLATE.get()));
        player.setItemSlot(EquipmentSlot.LEGS,  new ItemStack(ModItems.MARIKA_LEGGINGS.get()));
